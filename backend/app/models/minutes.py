...
class Minutes(SQLModel, table=True):
    ...
    agenda_id: Optional[int] = Field(default=None, foreign_key="agendas.id", index=True, ondelete="CASCADE")
    ...
    approved_by: Optional[int] = Field(default=None, foreign_key="user.id", ondelete="SET NULL")
...
class MinuteRevision(SQLModel, table=True):
    ...
    id: Optional[int] = Field(default=None, primary_key=True)
    minutes_id: int = Field(foreign_key="minutes.id", nullable=False, index=True, ondelete="CASCADE")
    version_no: int = Field(nullable=False)
    body: str = Field(default="", nullable=False)
    change_reason: Optional[str] = Field(default=None)
    changed_by: int = Field(foreign_key="user.id", nullable=False, index=True, ondelete="RESTRICT")
