...
import styles from "./repository-detail-view.module.css";

type RepositoryDetailViewProps = {
  contentId: string;
};

export function RepositoryDetailView({ contentId }: RepositoryDetailViewProps) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      try {
        ...
        const response = await apiFetch(`/api/v1/repository/${numericContentId}`);
  ...
  if (hasFetchError || !content) {
    return (
      <div className="container mx-auto px-4 py-8">
        ...
        <Link href="/repository" className="mt-4 inline-block text-blue-500 hover:underline">
          {t("common.back")}
        </Link>
      </div>
    );
        ...
        <main className={styles.main}>
          <div className={styles.breadcrumb}>
            <Link href="/" className={styles.breadcrumbLink}>
              {t("repositoryView.home")}
            </Link>
            <span className={styles.breadcrumbSeparator}>/</span>
            <Link href="/repository" className={styles.breadcrumbLink}>
              {t("repositoryView.title")}
            </Link>
            <span className={styles.breadcrumbSeparator}>/</span>
            <span className={styles.breadcrumbCurrent}>{t("repositoryView.current")}</span>
          </div>

          <section className={styles.hero}>
            <p className={styles.badge}>{t("repositoryView.badge")}</p>
            <h2 className={styles.title}>{t("repositoryView.title")}</h2>
            <p className={styles.description}>{t("repositoryView.description")}</p>
          </section>

          {isLoading ? <section className={styles.panel}>{t("repositoryView.loading")}</section> : null}
          {!isLoading && isNotFound ? <section className={styles.panel}>データが見つかりませんでした。</section> : null}
          {!isLoading && hasFetchError ? <section className={styles.panel}>{t("repositoryView.fetchFailed")}</section> : null}
